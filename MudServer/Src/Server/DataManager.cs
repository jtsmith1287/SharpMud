...
public static class DataManager {
    ...
    public static Dictionary<string, ItemDef> ItemDefRegistry {
        get { return ItemDef.Registry; }
        set { ItemDef.Registry = value; }
    }
            ...
            foreach (string path in paths) {
                ...
                switch (path) {
                    ...
                    case DataPaths.Creatures:
                        ...
                        break;
                    case DataPaths.Items:
                        json = serializer.Serialize(ItemDef.Registry);
                        break;
                }
            ...
            try {
                ...
                switch (path) {
                    ...
                    case DataPaths.World:
                        ...
                        foreach (Room room in World.World.Rooms.Values) {
                            if (room.EntitiesHere == null) {
                                room.EntitiesHere = new List<Guid>();
                            }

                            if (room.ItemsHereIds == null) continue;
                            foreach (string itemId in room.ItemsHereIds) {
                                if (!ItemDef.Registry.TryGetValue(itemId, out ItemDef def)) continue;
                    ...
                    case DataPaths.Creatures:
                        ...
                        break;
                    case DataPaths.Items:
                        ItemDef.Registry = serializer.Deserialize<Dictionary<string, ItemDef>>(json) ??
                                           new Dictionary<string, ItemDef>();
                        foreach (var kvp in ItemDef.Registry) {
    ...
    public static void UnifyExits() {
        Dictionary<string, Exit> exitRegistry = new Dictionary<string, Exit>();
        foreach (Room room in World.World.Rooms.Values) {
            if (room.Exits == null) continue;

            var exitKeys = room.Exits.Keys.ToList();
            foreach (var key in exitKeys) {
                Exit exit = room.Exits[key];
                if (exit == null) continue;

                string pathId = exit.GetPathId();
                if (string.IsNullOrEmpty(pathId)) continue;

                if (exitRegistry.TryGetValue(pathId, out Exit existingExit)) {
                    room.Exits[key] = existingExit;
                } else {
                    exitRegistry[pathId] = exit;
                }
            }
        }
    }
